...
/// @title SwapReleaser
/// @notice A contract that releases assets from an AssetSink in exchange for burning a threshold
/// amount of a resource token
/// @dev Inherits from FirepitImmutable for resource burning functionality and Nonce for replay
/// protection
contract SwapReleaser is FirepitImmutable, Nonce {
  ...
  /// @notice Creates a new SwapReleaser instance
  /// @param _resource The address of the resource token that must be burned
  ...
  function torch(uint256 _nonce, Currency[] memory assets, address recipient)
    ...
    handleNonce(_nonce)
  {
    RESOURCE.safeTransferFrom(msg.sender, RECIPIENT, THRESHOLD);
