...
/// @title ArbitrumOrbitResourceFirepit
/// @notice A releaser that implements a two-stage burn process for bridged resource tokens on
/// Arbitrum Orbit chains
/// @dev Two-stage burn from Arbitrum Orbit L2's to the underlying resource on L1
...
contract ArbitrumOrbitResourceFirepit is ExchangeReleaser {
  ...
  /// @dev Required for Arbitrum Orbit gateway calls which use L1 token address
  ...
  /// @notice Creates a new ArbitrumOrbitResourceFirepit instance
  /// @param _resource The address of the resource token on Arbitrum Orbit chain (L2 bridged token)
  ...
  constructor(
    ...
    L2_GATEWAY_ROUTER = _l2GatewayRouter;
  ...
  function _afterRelease(Currency[] calldata, address) internal override {
    ...
    IL2GatewayRouter(L2_GATEWAY_ROUTER)
      .outboundTransfer(
        L1_RESOURCE, // L1 token address (required by Arbitrum Orbit chain gateway)
