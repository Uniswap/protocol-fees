...
/// @param isService A boolean flag that is part of the log along with `key`, `value`, and `sender`
/// address. This field is required formally but does not have any special meaning
...
interface IBridgehub {
  ...
  /// @dev Also emitted when new admin is accepted and in this case, `newPendingAdmin` would be zero
  /// address
  ...
  /// @notice Starts the transfer of admin rights. Only the current admin or owner can propose a new
  /// pending one. @notice New admin can accept admin rights by calling `acceptAdmin` function.
  ...
  event NewChain(
    uint256 indexed chainId, address chainTypeManager, address indexed chainGovernance
  );
  ...
  function forwardedBridgeMint(bytes32 _assetId, uint256 _chainId, bytes32 _baseTokenAssetId)
    external
    returns (address zkChain, address ctm);

  function registerNewZKChain(uint256 _chainId, address _zkChain, bool _checkMaxNumberOfZKChains)
    external;
