...
// NOTICE:
//
// This deployment script necessitates a balance of the native token (Ether's equivalent on Polygon)
// both to pay for gas **and** to pay for a wormhole core message.
//
// cast call 0x7A4B5a56256163F07b2C80A7cA55aBE66c4ec4d7 "messageFee()(uint256)" --rpc-url
// https://polygon-bor-rpc.publicnode.com
// Wormhole: https://polygonscan.com/address/0x7A4B5a56256163F07b2C80A7cA55aBE66c4ec4d7
// Implementation: https://polygonscan.com/address/0x381752f5458282d317d12C30D2Bd4D6E1FD8841e
//
// ---
//
// UPDATE: This appears to return `0` on Polygon, so this may be unnecessry. It's unclear whether
...
//
contract DeployWormholeInfraPolygonScript is Script {
  ...
  function run() public {
    ...
    //
    // - `_token`: Polygon deployment of UNI.
    ...
    nttManagerImplementation = address(
      new NttManagerNoRateLimiting({
        ...
        _chainId: Constants.Wormhole.POLYGON_CHAIN_ID
      })
    );
    ...
    wormholeTransceiverImplementation = address(
      new WormholeTransceiver({
        ...
        wormholeCoreBridge: Constants.Polygon.WORMHOLE,
    ...
    //
    wormholeTransceiverProxy = address(
    ...
    //
    NttManagerNoRateLimiting(nttManagerProxy)
      .setTransceiver({transceiver: wormholeTransceiverProxy});
    ...
    //
    // [1]
    // https://github.com/wormhole-foundation/native-token-transfers/blob/25a5c3f89446be499a89d1c453db996f29de9290/evm/script/helpers/DeployWormholeNttBase.sol#L133
    // 
    NttManagerNoRateLimiting(nttManagerProxy).setThreshold({threshold: 1});
    ...
    //
    SyntheticNttUni(syntheticNttUni).setNtt({newNtt: nttManagerProxy});
    ...
    //
    SyntheticNttUni(syntheticNttUni).transferOwnership({newOwner: Constants.Polygon.ETHEREUM_PROXY});
    ...
    console2.log(
      "NttManager (Implementation)                           : ", nttManagerImplementation
    );
    console2.log(
      "WormholeTransceiver (ERC1967Proxy)                    : ", wormholeTransceiverProxy
    );
    console2.log(
      "WormholeTransceiver (Implementation)                  : ", wormholeTransceiverImplementation
    );
    console2.log("\n");
    ...
    console2.log(
      "syntheticNttUni.ntt()                                     : ",
      SyntheticNttUni(syntheticNttUni).ntt()
    );
    console2.log("nttManagerProxy                                           : ", nttManagerProxy);
    console2.log("\n");

    console2.log(
      "syntheticNttUni.owner()                                   : ",
      SyntheticNttUni(syntheticNttUni).owner()
    );
    console2.log(
    ...
    console2.log(
      "nttManagerProxy ERC1967 Implementation                    : ",
      readImplementation(nttManagerProxy)
    );
    console2.log(
    ...
    console2.log(
      "nttManagerProxy.getMode()                                 : ",
      NttManagerNoRateLimiting(nttManagerProxy).getMode()
    );
    console2.log(
    ...
    console2.log(
      "nttManagerProxy.token()                                   : ",
      NttManagerNoRateLimiting(nttManagerProxy).token()
    );
    console2.log("SyntheticNttUni                                           : ", syntheticNttUni);
    console2.log("\n");

    console2.log(
      "nttManagerProxy.getThreshold()                            : ",
      NttManagerNoRateLimiting(nttManagerProxy).getThreshold()
    );
    console2.log("NttManager Threshold                                      : ", uint8(1));
    console2.log("\n");

    console2.log(
      "nttManagerProxy.getTransceiverInfo().length               : ", transceiverInfos.length
    );
    console2.log("NttManager Transceiver Count                              : ", uint256(1));
    console2.log("\n");

    console2.log(
      "nttManagerProxy.getTransceiverInfo()[0].registered        : ", transceiverInfos[0].registered
    );
    console2.log("NttManager Transceiver 0 Registered                       : ", true);
    console2.log("\n");

    console2.log(
      "nttManagerProxy.getTransceiverInfo()[0].enabled           : ", transceiverInfos[0].enabled
    );
    console2.log("NttManager Transceiver 0 Enabled                          : ", true);
    console2.log("\n");

    console2.log(
      "nttManagerProxy.getTransceiverInfo()[0].index             : ", transceiverInfos[0].index
    );
    console2.log("NttManager Transceiver 0 Index                            : ", uint8(0));
    console2.log("\n");

    console2.log(
      "wormholeTransceiverProxy ERC1967 Implementation           : ",
      readImplementation(wormholeTransceiverProxy)
    );
    console2.log(
    ...
    console2.log(
      "wormholeTransceiverProxy.nttManager()                     : ",
      WormholeTransceiver(wormholeTransceiverProxy).nttManager()
    );
    console2.log("NttManager Proxy                                          : ", nttManagerProxy);
    console2.log("\n");

    console2.log(
      "wormholeTransceiverProxy.nttManagerToken()                : ",
      WormholeTransceiver(wormholeTransceiverProxy).nttManagerToken()
    );
    console2.log("SyntheticNttUni                                           : ", syntheticNttUni);
    console2.log("\n");

    console2.log(
      "wormholeTransceiverProxy.consistencyLevel()               : ",
      WormholeTransceiver(wormholeTransceiverProxy).consistencyLevel()
    );
    console2.log("Consistency Level (Hard coded)                            : ", uint8(202));
    console2.log("\n");

    console2.log(
      "wormholeTransceiverProxy.customConsistencyLevel()         : ",
      WormholeTransceiver(wormholeTransceiverProxy).customConsistencyLevel()
    );
    console2.log("Custom consistency Level (Hard coded)                     : ", uint8(0));
    console2.log("\n");

    console2.log(
      "wormholeTransceiverProxy.additionalBlocks()               : ",
      WormholeTransceiver(wormholeTransceiverProxy).additionalBlocks()
    );
    console2.log("Additional blocks (Hard coded)                            : ", uint16(0));
    console2.log("\n");

    console2.log(
      "wormholeTransceiverProxy.customConsistencyLevelAddress()  : ",
      WormholeTransceiver(wormholeTransceiverProxy).customConsistencyLevelAddress()
    );
    console2.log("Custom consistency level address                          : ", address(0x00));
    console2.log("\n");

    console2.log(
      "wormholeTransceiverProxy.wormhole()                       : ",
      address(WormholeTransceiver(wormholeTransceiverProxy).wormhole())
    );
    console2.log(
      "Wormhole                                                  : ", Constants.Polygon.WORMHOLE
    );
    console2.log("\n");
    ...
    require(
      WormholeTransceiver(wormholeTransceiverProxy).customConsistencyLevelAddress() == address(0x00)
    );
    require(
      address(WormholeTransceiver(wormholeTransceiverProxy).wormhole())
