...
contract ConfigWormholeInfraPolygonScript is Script {
  ...
  function run() public {
    ...
    NttManagerNoRateLimiting(polygon.nttManagerProxy)
      .setPeer({
        peerChainId: Constants.Wormhole.ETH_CHAIN_ID,
    ...
    //
    NttManagerNoRateLimiting(polygon.nttManagerProxy)
      .transferOwnership({newOwner: Constants.Polygon.ETHEREUM_PROXY});
    ...
    //
    address transceiverPeer = address(
      uint160(
        uint256(
          WormholeTransceiver(polygon.wormholeTransceiverProxy)
    ...
    console2.log(
      "eth.wormholeTransceiverProxy                        : ", eth.wormholeTransceiverProxy
    );
    console2.log("\n");

    console2.log(
      "polygon.nttManagerProxy.getPeer().peerAddress       : ",
      address(uint160(uint256(nttManagerPeer.peerAddress)))
    );
    console2.log("eth.nttManagerProxy                                 : ", eth.nttManagerProxy);
    console2.log("\n");

    console2.log(
      "polygon.nttManagerProxy.getPeer().tokenDecimals     : ", nttManagerPeer.tokenDecimals
    );
    console2.log("eth.uni.decimals()                                  : ", uint8(18));
    console2.log("\n");

    console2.log(
      "polygon.wormholeTransceiverProxy.owner()            : ",
      WormholeTransceiver(polygon.wormholeTransceiverProxy).owner()
    );
    console2.log("Constants.Polygon.ETHEREUM_PROXY : ", Constants.Polygon.ETHEREUM_PROXY);
    console2.log("\n");

    console2.log(
      "polygon.nttManagerProxy.owner()                     : ",
      NttManagerNoRateLimiting(polygon.nttManagerProxy).owner()
    );
    console2.log("Constants.Polygon.ETHEREUM_PROXY : ", Constants.Polygon.ETHEREUM_PROXY);
    console2.log("\n");

    console2.log(
      "polygon.nttManagerProxy.pauser()                    : ",
      NttManagerNoRateLimiting(polygon.nttManagerProxy).pauser()
    );
    ...
    require(
      NttManagerNoRateLimiting(polygon.nttManagerProxy).owner() == Constants.Polygon.ETHEREUM_PROXY
    );
  ...
  function loadDeployments() internal {
    ...
    require(
      polygon.nttManagerImplementation.code.length != 0, "NttManager implementation has no code."
    );
    require(polygon.nttManagerProxy.code.length != 0, "NttManager proxy has no code.");
    require(
      polygon.wormholeTransceiverImplementation.code.length != 0,
      "WormholeTransceiver implementation has no code."
    );
    require(
      polygon.wormholeTransceiverProxy.code.length != 0, "WormholeTransceiver proxy has no code."
    );

    // Check UNI metadata.
    //
    require(
      keccak256(bytes(SyntheticNttUni(polygon.uni).name())) == keccak256("Synthetic Ntt Uniswap"),
      "polygon.uni.name() mismatch"
    );
    require(
      keccak256(bytes(SyntheticNttUni(polygon.uni).symbol())) == keccak256("NUNI"),
      "polygon.uni.symbol() mismatch"
    );
    require(SyntheticNttUni(polygon.uni).decimals() == 18, "polygon.uni.decimals() mismatch");
    require(
      SyntheticNttUni(polygon.uni).ntt() == polygon.nttManagerProxy, "polygon.uni.ntt() mismatch"
    );
    require(
      SyntheticNttUni(polygon.uni).owner() == Constants.Polygon.ETHEREUM_PROXY,
      "polygon.uni.owner() mismatch"
    );
    ...
    require(
      readImplementation(polygon.nttManagerProxy) == polygon.nttManagerImplementation,
      "polygon.nttManagerProxy.implementation() mismatch"
    );
    require(
      NttManagerNoRateLimiting(polygon.nttManagerProxy).getMode()
        == uint8(IManagerBase.Mode.BURNING),
      "polygon.nttManagerProxy.mode() mismatch"
    );
    require(
      NttManagerNoRateLimiting(polygon.nttManagerProxy).token() == polygon.uni,
      "polygon.nttManagerProxy.token() mismatch"
    );
    require(
      NttManagerNoRateLimiting(polygon.nttManagerProxy).getThreshold() == 1,
      "polygon.nttManagerProxy.getThreshold() mismatch"
    );
    ...
    //
    require(
      readImplementation(polygon.wormholeTransceiverProxy)
        == polygon.wormholeTransceiverImplementation,
      "polygon.nttManagerProxy.implementation() mismatch"
    );
    require(
      WormholeTransceiver(polygon.wormholeTransceiverProxy).nttManager() == polygon.nttManagerProxy,
      "wormholeTransceiverProxy.nttManager() mismatch"
    );
    require(
      WormholeTransceiver(polygon.wormholeTransceiverProxy).nttManagerToken() == polygon.uni,
      "wormholeTransceiverProxy.nttManagerToken() mismatch"
    );
    require(
      WormholeTransceiver(polygon.wormholeTransceiverProxy).consistencyLevel() == 202,
      "wormholeTransceiverProxy.consistencyLevel() mismatch"
    );
    require(
      WormholeTransceiver(polygon.wormholeTransceiverProxy).customConsistencyLevel() == 0,
      "wormholeTransceiverProxy.customConsistencyLevel() mismatch"
    );
    require(
      WormholeTransceiver(polygon.wormholeTransceiverProxy).additionalBlocks() == 0,
      "wormholeTransceiverProxy.additionalBlocks() mismatch"
    );
    require(
      WormholeTransceiver(polygon.wormholeTransceiverProxy).customConsistencyLevelAddress()
        == address(0x00),
      "wormholeTransceiverProxy.customConsistencyLevelAddress() mismatch"
    );
    require(
      address(WormholeTransceiver(polygon.wormholeTransceiverProxy).wormhole())
