...
contract DeployV4FeeInfra is Script {
  ...
  function run() external {
    ...
    deployAndConfigureV4Infra(
      ChainId.Ethereum, uniswap.ethereum.poolManager, uniswap.ethereum.tokenJar
    );
    deployAndConfigureV4Infra(
      ChainId.Arbitrum, uniswap.arbitrum.poolManager, uniswap.arbitrum.tokenJar
    );
    ...
    deployAndConfigureV4Infra(
      ChainId.Optimism, uniswap.optimism.poolManager, uniswap.optimism.tokenJar
    );
    deployAndConfigureV4Infra(
      ChainId.Soneium, uniswap.soneium.poolManager, uniswap.soneium.tokenJar
    );
    deployAndConfigureV4Infra(ChainId.XLayer, uniswap.xLayer.poolManager, uniswap.xLayer.tokenJar);
    deployAndConfigureV4Infra(
      ChainId.WorldChain, uniswap.worldChain.poolManager, uniswap.worldChain.tokenJar
    );
    deployAndConfigureV4Infra(ChainId.Zora, uniswap.zora.poolManager, uniswap.zora.tokenJar);
    deployAndConfigureV4Infra(
      ChainId.BNBChain, uniswap.bnbChain.poolManager, uniswap.bnbChain.tokenJar
    );
    deployAndConfigureV4Infra(
      ChainId.Polygon, uniswap.polygon.poolManager, uniswap.polygon.tokenJar
    );

    vm.stopBroadcast();
  }

  function deployAndConfigureV4Infra(uint256 chainId, address poolManager, address tokenJar)
    internal
  {
    ...
    //
    {
      ...
      for (uint256 i; i < length; i++) {
        ...
        (token0, token1) = uint160(token0) >= uint160(token1) ? (token1, token0) : (token0, token1);
