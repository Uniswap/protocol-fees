...
contract WormholeReleaserIntegrationTest is Test {
    ...
    function setUp() public {
        ...
        _deploy({
            ...
            wormholeChainId: WormholeID.ETHEREUM
        });
        _deploy({
            net: pol,
            mode: IManagerBase.Mode.BURNING,
            wormholeChainId: WormholeID.POLYGON
        });

        _config({
            src: pol,
            dst: eth,
            wormholePeerChainId: WormholeID.ETHEREUM
        });
        _config({
            src: eth,
            dst: pol,
            wormholePeerChainId: WormholeID.POLYGON
        });
    ...
    function testFuzzRelease(FuzzParams memory fuzz) external {
        ...
        shouldSucceed = shouldSucceed && (fuzz.amountPaid == fuzz.wormholeFee || !fuzz.shouldCallerRefundThrow);
    ...
    function _deploy(
        ...
        NttManagerNoRateLimiting.TransceiverInfo[]
            memory transceiverInfos = net.nttManager.getTransceiverInfo();
    ...
    function _config(
        Deployment storage src,
        Deployment storage dst,
        uint16 wormholePeerChainId
    ) internal {
        src.transceiver.setWormholePeer({
            peerChainId: wormholePeerChainId,
            peerContract: addrB32(address(dst.transceiver))
        });

        src.nttManager.setPeer({
            peerChainId: wormholePeerChainId,
        ...
        NttManagerNoRateLimiting.NttManagerPeer memory nttManagerPeer = src.nttManager.getPeer(wormholePeerChainId);
