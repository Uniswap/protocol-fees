...
contract PreflightCheckTest is Test {
  ...
  function testProtocolState() public {
    ...
    // Core
    //
    assertEq(IUniswapV2Factory(Constants.Celo.V2_FACTORY).feeTo(), address(0x00));
    assertEq(
    ...
    // Core
    //
    assertEq(IUniswapV2Factory(Constants.BNB.V2_FACTORY).feeTo(), address(0x00));
    assertEq(
    ...
    // Core
    //
    assertEq(IUniswapV2Factory(Constants.Polygon.V2_FACTORY).feeTo(), address(0x00));
    assertEq(
      IUniswapV2Factory(Constants.Polygon.V2_FACTORY).feeToSetter(),
      Constants.Polygon.ETHEREUM_PROXY
    );
    assertEq(
      IUniswapV3Factory(Constants.Polygon.V3_FACTORY).owner(), Constants.Polygon.ETHEREUM_PROXY
    );
  }
}
