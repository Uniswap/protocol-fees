...
contract ExchangeReleaserTest is PhoenixTestBase {
  ...
  function test_torch_release_erc20() public {
    ...
    swapReleaser.torch(swapReleaser.nonce(), releaseMockToken, alice);
  ...
  function test_torch_release_native() public {
    ...
    vm.startPrank(alice);
    resource.approve(address(swapReleaser), INITIAL_TOKEN_AMOUNT);
    swapReleaser.torch(swapReleaser.nonce(), releaseMockNative, alice);
  ...
  function test_fuzz_revert_torch_insufficient_balance(uint256 amount, uint256 seed) public {
    ...
    swapReleaser.torch(nonce, fuzzReleaseAny[seed % fuzzReleaseAny.length], alice);
  }
  ...
  function test_revert_torch_frontrun() public {
    ...
    // First torch call
    swapReleaser.torch(nonce, releaseMockToken, alice);
    assertEq(mockToken.balanceOf(alice), INITIAL_TOKEN_AMOUNT);
    assertEq(mockToken.balanceOf(address(assetSink)), 0);
    assertEq(resource.balanceOf(alice), 0);
    assertEq(resource.balanceOf(address(swapReleaser)), 0);
